...
exports.derivative = (req, res) => {
	const { function: func, variable } = req.body;
	...
	try {
		...
		if (!func || !variable) {
			return res
				.status(400)
				.json({ error: 'Please provide a valid function and variable.' });
		}
		// calculate the derivative of a function with respect to the given variable
		const derivative = math.derivative(func, variable);

		// return the result in a simplified form
		res.json({ result: derivative.toTex() });
	} catch (err) {
		res
			.status(500)
			.json({ error: 'Error calculating derivative. Check your input.' });
	}
};
